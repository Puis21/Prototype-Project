...
#include "Notify_QueueNextAttack.h"
#include "PrototypeProject/Player/Components/CombatComponent.h"

void UNotify_QueueNextAttack::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation)
{
	if (nullptr != MeshComp)
	{
		...
		if (nullptr != Owner)
		{
			...
			{
				m_pAMeleeComp->NextAttackedQueued();
			}
		}
	}
}